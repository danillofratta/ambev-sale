...
public class SaleDiscountServiceTests
{
    ...
    public void ValidateSaleItems_ValidItems_CorrectlyAppliesDiscount()
    {
        ...
        //foreach (var item in items)
        //{
        //    if (item.Quantity >= 10)// SaleDiscountService.MID_DISCOUNT_QUANTITY)
        //    {
        //        Assert.Equal(0.20m, item.Discount / (item.Quantity * item.UnitPrice)); // Bulk discount applied
        //    }
        //    else
        //    {
        //        Assert.Equal(0.10m, item.Discount / (item.Quantity * item.UnitPrice)); // Basic discount applied
        //    }
        //}
    }
